...
class _AddReportViewPageState extends State<AddReportViewPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AddReportBloc(context.read<Repository>())
        ..add(GetAddReportEvent(widget.id)),
      child: Scaffold(
  ...
  TableRow tableRowCheckBox(String title, bool isError, int id, List<Select> listSelected,
      ...
      if(list[i].questionType == "simple_choice" || list[i].questionType == "multiple_choice"
        ...
        if(list[i].questionAndPageIds.length>0){
          ...
          for(int i = 0 ; i < listParent.length; i++) {
            print("HoangCCV: chac chan chạy vào đây đc");
            List<ObjectTable> listWidget = [];
            form = 5;
              for (int j = 0; j <
                ...
                if (listParent[i].suggestedAnswerIds[j].questionAndPageIds
                  ...
                  // tao thang con : Bao goi/cach khac
                    print("HoangCCV: chac chan chạy vào đây đc: ${listParent[i].suggestedAnswerIds[j].value}");
                  ...
                  bool hasCheckBox = listParent[i].suggestedAnswerIds[j].questionAndPageIds.any((element) => element.questionType == "check_box");
                    // tao thang con: bao goi cua nong dan
                  for (int k = 0; k <
          ...
          for (int j = 0; j < list[i].suggestedAnswerIds.length; j ++) {
            if (list[i].suggestedAnswerIds[j].questionAndPageIds.isNotEmpty){
              ...
              listWidget.add(ObjectTable(tableRowCheckBox("${list[i].suggestedAnswerIds[j].value}",
                ...
                print("HoangC:V: chắc chắn chạy vào đây");
                //Tao thang con
              for (int k = 0; k <
...
/*                  print("HoangCV: rowID:  ${a} : ${listTableField[index].listQuestion[a]
      ...
      if(list[i].questionType  == "table" && tableWidgetText.isNotEmpty){
        // table
...
class ObjectTable {
  ...
  ObjectTable(this.tableRow, {this.list = const []});
}
